def JSON_to_HL7(patient):
    ...
    # MSH segment
    msh_segment = f"MSH|^~\&||||{patient.get('fecha', '')}|||ADT^A01|||2.6"
    hl7_message.append(msh_segment)

    # PID segment
    pid_segment = f"PID|||{patient.get('ID', '')}|||{patient.get('fecha', '')}|{patient.get('sexo', '')}||{patient.get('apellidos', '')}^{patient.get('nombres', '')}"
    hl7_message.append(pid_segment)

    # OBR segment
    for i, examen in enumerate(patient.get('examenes', []), start=1):
